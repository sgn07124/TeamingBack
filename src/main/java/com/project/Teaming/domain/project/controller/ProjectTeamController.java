...
public class ProjectTeamController {
    ...
    @GetMapping("/project/teams/{teamId}")
    @Operation(summary = "프로젝트 팀 정보 조회", description = "특정 프로젝트 팀의 정보를 조회한다.")
    public ResultDetailResponse<ProjectTeamInfoDto> getTeam(@PathVariable Long teamId) {
        ProjectTeamInfoDto dto = projectTeamService.getTeam(teamId);
        return new ResultDetailResponse<>(ResultCode.GET_PROJECT_TEAM, dto);
    }

    @PutMapping("/project/teams/{teamId}")
    @Operation(summary = "프로젝트 팀 정보 수정", description = "프로젝트 팀의 정보를 수정한다.")
    public ResultDetailResponse<Void> editTeam(@PathVariable Long teamId, @RequestBody UpdateTeamDto updateTeamDto) {
        projectTeamService.editTeam(teamId, updateTeamDto);
        return new ResultDetailResponse<>(ResultCode.UPDATE_PROJECT_TEAM, null);
    }

    @DeleteMapping("project/teams/{teamId}")
    @Operation(summary = "프로젝트 팀 삭제", description = "프로젝트 팀을 삭제한다.")
    public ResultDetailResponse<Void> deleteTeam(@PathVariable Long teamId) {
        projectTeamService.deleteTeam(teamId);
        return new ResultDetailResponse<>(ResultCode.DELETE_PROJECT_TEAM, null);
    }

    @PutMapping("/project/teams/status")
